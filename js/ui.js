  ...
  return `
    <div class="quick-feed-layer" role="dialog" aria-modal="true" aria-labelledby="quick-feed-title">
      ...
      <section class="quick-feed-sheet">
        ...
        ${ownedFoods.length ? `
          <div id="quick-feed-food-grid" class="quick-feed-food-grid" role="list" aria-label="已有庫存的食物">
            ${ownedFoods.map((food) => {
              ...
              return `
                <button type="button" class="quick-feed-food-card ${isSelected ? "is-selected" : ""}" data-action="quick-feed-select" data-food-id="${food.id}" aria-pressed="${isSelected}">
                  ...
                  <span class="quick-feed-food-card-copy">
                    ...
                    <em data-quick-feed-stock="${food.id}">EXP +${formatNumber(food.exp)} · 庫存 ×${formatNumber(quantity)}</em>
                  </span>
                </button>
              `;
            }).join("")}
          ...
          <div class="quick-feed-selection">
            ...
            <div class="quantity-control quick-feed-quantity-control" aria-label="餵食數量">
              ...
              <input id="quick-feed-quantity-input" type="number" min="1" max="${selectedStock}" step="1" inputmode="numeric" value="${selectedQuantity}" data-action="quick-feed-quantity-input" data-food-id="${selectedFood.id}" aria-label="餵食數量，${escapeHtml(selectedFood.shortName)}" />
...
export function renderHome(container, save, options = {}) {
  ...
  const selectedAccessoryId = options.selectedAccessoryId || null;
      ...
      <section class="jelly-card glass-card">
        ...
        <div id="jelly-stage" class="jelly-stage ${scene.cssClass}" data-stage="${stage.stage}">
          ...
          <div id="jelly-display" class="jelly-display">
            ${renderJellyfish(save, { accessoryEditMode, selectedAccessoryId })}
          </div>
        </div>
